...
class donor:
    def __init__(self, root):
        ...
        # manage title
        M_title = Label(Manage_frame, text="MANAGE DONOR", font=("times new roman", 20, "bold"), fg="black",bg="RosyBrown1", bd=3, relief=RAISED)
        ...
        add_btn = Button(btn_frame, text="ADD", command=self.add_donor, width=11, height=2, fg="black", bg="seashell")
        add_btn.grid(row=0, column=0, padx=10, pady=10)

        update_btn = Button(btn_frame, text="UPDATE", command=self.update_donor, width=11, height=2, fg="black", bg="seashell")
        update_btn.grid(row=0, column=1, padx=10, pady=10)

        delete_btn = Button(btn_frame, text="DELETE", command=self.delete_donor, width=11, height=2, fg="black", bg="seashell")
        ...
        search_btn = Button(Details_frame, command=self.search_donor, text="Search", width=8, height=2, fg="black", bg="seashell")
        ...
        self.donor_table = ttk.Treeview(Table_frame, column=("id", "name", "email", "dob", "gender", "contact", "address"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.donor_table.heading("id", text="Id")
        self.donor_table.heading("name", text="Name")
        self.donor_table.heading("email", text="Email")
        self.donor_table.heading("dob", text="DOB")
        ...
        self.donor_table.column("id", width=90)
        self.donor_table.column("name", width=130)
        self.donor_table.column("email", width=130)
        self.donor_table.column("dob", width=100)
        self.donor_table.column("gender", width=100)
        self.donor_table.column("contact", width=100)
        self.donor_table.column("address", width=180)
    ...
    def add_donor(self):
        ...
        my_cursor.execute("insert into donor values(%s,%s,%s,%s,%s,%s,%s)", (
                                                                            self.id_var.get(),
                                                                            self.name_var.get(),
                                                                            self.email_var.get(),
                                                                            self.gender_var.get(),
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="Loveurlife762", database="orphanage")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from donor")
        rows = my_cursor.fetchall()
        if len(rows) != 0:
            self.donor_table.delete(*self.donor_table.get_children())
            for i in rows:
                self.donor_table.insert("", END, values=i)
            conn.commit()
            conn.close()
    ...
    def get_cursor(self,event=""):
        cursor_row=self.donor_table.focus()
        content=self.donor_table.item(cursor_row)
        row=content["values"]
    ...
    def update_donor(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="Loveurlife762", database="orphanage")
        my_cursor = conn.cursor()
        my_cursor.execute("update donor set name=%s,email=%s,gender=%s,dob=%s,contact=%s,address=%s where id=%s", (
                                                                                                                    self.name_var.get(),
                                                                                                                    self.email_var.get(),
                                                                                                                    self.gender_var.get(),
                                                                                                                    self.dob_var.get(),
    ...
    def search_donor(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="Loveurlife762", database="orphanage")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from donor where "+str(self.search_by.get())+"LIKE"+str(self.search_txt.get())+"%")
        rows = my_cursor.fetchall()
